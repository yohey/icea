...
prob.set_problem(mode = 'hp', name = 'Example 5')

prob.set_output_options(SI = False)

prob.add_reactant('name', 'NH4CLO4(I)',  ratio = 72.06, T = 298.15)
prob.add_reactant('name', 'CHOS-Binder', formula = 'C 1 H 1.86955 O .031256 S .008415',
                                         ratio = 18.58, T = 298.15, h = -2999.082, h_unit = 'cal/mol')
prob.add_reactant('name', 'AL(cr)',      ratio =  9.00, T = 298.15)
prob.add_reactant('name', 'MgO(cr)',     ratio =  0.20, T = 298.15)
prob.add_reactant('name', 'H2O(L)',      ratio =  0.16, T = 298.15)

prob.set_chamber_pressures([500, 250, 125, 50, 5], unit = 'legacy-psi')
...
prob.run(out_filename = 'example-05.out')
