...
! LOCAL VARIABLES
 ...
 400  Tln = dlog(Tt)
      ...
      if( Npt == 1 .and. .not. Shock .and. .not. Short ) then
         write(IOOUT, '(/" POINT ITN", 6X, "T", 10X, 4(A4, 8X)/(18X, 5(A4, 8X)))') &
              (Elmt(i), i=1, Nlm)
      end if
      if( Debug(Npt) ) then
        do i = 1, Nlm
...
! ERROR, SET TT=0
 ...
 710        continue
          ...
          do inc = 1, Nc
            j = inc + Ng
            if( Debug(Npt) ) write(IOOUT, '(/1X, A15, 2F10.3, 3X, E15.7)') Prod(j), Temp(1, inc), &
                                     Temp(2, inc), En(j, Npt)
            if( En(j, Npt) <= 0. ) then
...
! WRONG PHASE INCLUDED FOR T INTERVAL, SWITCH EN
 740      En(jkg, Npt) = En(j, Npt)
          ...
          En(j, Npt) = 0.
          jsw = j
          if( Prod(j) /= Prod(jkg) .and. .not. Short ) write(IOOUT, '(" PHASE CHANGE, REPLACE ", A16, "WITH ", A16)') &
               Prod(j), Prod(jkg)
          j = jkg
          go to 900
        end if
